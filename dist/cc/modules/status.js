import { MoneyFormat, TimeFormat, Hacks, Doc } from "../constants.js";
...
export class Status extends Module {
    ...
    async update(command, status) {
        ...
        table(["Color", "Memory", "Cost", "Command"], costs.map(c => c.color), costs.map(c => `${c.size}GB`), costs.map(c => this.ns.nFormat(c.cost, MoneyFormat)), costs.map(c => `buyServer --mem ${c.size} --count #`)).split("\n").forEach(line => {
            this.ns.print(line);
        });
        ...
        table(["Color", "Lv", "Hostname", "Memory", "Usage", "Cur $", "Max $", "Security", "Growth", "Hack Time", "Grow Time", "Weaken Time", "Hack Chance"], servers.map(s => {
            if (s.hasAdminRights) {
                return "SUCCESS";
            }
            else {
                if (s.numOpenPortsRequired <= Hacks.map(hack => this.ns.fileExists(hack) ? 1 : 0).reduce((a, b) => a + b)) {
                    return "WARN";
                }
                else {
                    return "ERROR";
        ...
        }), servers.map(s => `${s.requiredHackingSkill}`), servers.map(s => s.hostname), servers.map(s => `${s.maxRam}GB`), servers.map(s => s.maxRam === 0 ? "-" : `${(s.ramUsed / s.maxRam * 100).toFixed(0)}%`), servers.map(s => s.moneyMax === 0 ? "-" : this.ns.nFormat(s.moneyAvailable, MoneyFormat)), servers.map(s => s.moneyMax === 0 ? "-" : this.ns.nFormat(s.moneyMax, MoneyFormat)), servers.map(s => s.moneyMax === 0 ? "-" : `${s.minDifficulty.toFixed(1)}/${s.hackDifficulty.toFixed(1)}`), servers.map(s => s.moneyMax === 0 ? "-" : `${s.serverGrowth}`), servers.map(s => s.moneyMax === 0 ? "-" : this.ns.nFormat(this.ns.getHackTime(s.hostname) / 1000, TimeFormat)), servers.map(s => s.moneyMax === 0 ? "-" : this.ns.nFormat(this.ns.getGrowTime(s.hostname) / 1000, TimeFormat)), servers.map(s => s.moneyMax === 0 ? "-" : this.ns.nFormat(this.ns.getWeakenTime(s.hostname) / 1000, TimeFormat)), servers.map(s => s.moneyMax === 0 ? "-" : `${(this.ns.hackAnalyzeChance(s.hostname) * 100).toFixed(2)}%`)).split("\n").forEach(line => {
            this.ns.print(line);
        });
        // Sidebar
        try {
