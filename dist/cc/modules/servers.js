...
export class Servers extends Module {
    ...
    async update(command, status) {
        ...
        if (status && status.runtime.nodes.length > 0) {
            ...
            this.ns.print(table(["Hostname", "T", "Target", "Action", "Time Left", "Bar", "Result"], nodes.map(s => s.server.hostname), nodes.map(s => `${s.threads}`), nodes.map(s => s.target), nodes.map(s => s.action), nodes.map(s => {
                ...
                if (isNaN(fill) || isNaN(drain) || !isFinite(fill) || !isFinite(drain) || fill < 0 || drain < 0)
