...
const StartFlags: Flags = [["fresh", false]];
...
class ModuleRef {
	...
	constructor(name: string, script: string, threads: number, port: number) {
		this.name = name;
		this.script = script;
...
class Node {
	...
	target: string;
	server: Server;
	action: "Weaken" | "Grow" | "Hack" | "Idle" = "Weaken"
	log: string = "";

	constructor(pid: number, threads: number, target: string, server: Server) {
		this.pid = pid;
		this.threads = threads;
		this.target = target;
		this.server = server;
