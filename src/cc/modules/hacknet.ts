...
export async function main(ns: NS) {
	...
	isRunning = true;
	const hacknetModule = new HackNet(ns, ns.args[0] as number);
	ns.tail();
...
export class HackNet extends Module {
	...
	async update(command: any | null, status: {state: CCState, runtime: CCRuntime} | null) {
		...
		this.ns.print(table(
			...
			nodes.map((v, i) => this.hacknet.getNodeStats(i).name),
			nodes.map((v, i) => this.ns.nFormat(this.hacknet.getNodeStats(i).totalProduction, MoneyFormat)),
			nodes.map((v, i) => this.ns.nFormat(this.hacknet.getNodeStats(i).timeOnline, TimeFormat)),
			nodes.map((v, i) => `${this.ns.nFormat(this.hacknet.getNodeStats(i).production, MoneyFormat)} /s`),
			nodes.map((v, i) => `${this.hacknet.getNodeStats(i).level}`),
			nodes.map((v, i) => `${this.hacknet.getNodeStats(i).ram}`),
			nodes.map((v, i) => `${this.hacknet.getNodeStats(i).cores}`),
		));
