...
export async function main(ns) {
	...
	while(true) {
		...
		if(serverInfo.hackDifficulty > securityThreshold) {
			await ns.tryWritePort(18, JSON.stringify({hostname: host, action: "Weaken", startTime: Date.now()}));
			...
			await ns.tryWritePort(18, JSON.stringify({hostname: host, action: "Grow", startTime: Date.now()}));
			...
			await ns.tryWritePort(18, JSON.stringify({hostname: host, action: "Hack", startTime: Date.now()}));
